...
class TrieNode{
  ...
  List<Pair> getAllDescendentWords(String word){
    List<Pair> descendent = new ArrayList<>();
    if(isEndOfWord){
      descendent.add(new Pair(word, accessFrequency));
    }
    for(char c:children.keySet()){
      String newPrefix = word+c;
      descendent.addAll(children.get(c).getAllDescendentWords(newPrefix));
    }
    return descendent;
  }
